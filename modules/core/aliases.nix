...
let
  ...
  linuxOnly = if isLinux then {
    ...
    USER = "hx /etc/nixos/modules/users.nix";
    ALIAS = "hx /etc/nixos/modules/shell/aliases.nix";
    SYSINSTALL = "hx /etc/nixos/modules/system-packages.nix";
...
in common // linuxOnly // darwinOnly // wslOnly
