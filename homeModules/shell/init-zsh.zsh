...
DMUSIC() {
    ...
    if [[ -z "$1" ]]; then
        echo 'Usage:'
        echo '  Single Video: DMUSIC "<url>"'
        echo '  Playlist    : DMUSIC playlist "<url>"'
        return 1
    fi
    ...
    # 2. Penentuan Mode (Playlist vs Single) & Target Direktori
    if [[ "$1" == "playlist" ]]; then
        if [[ -z "$2" ]]; then
            echo 'Error: URL playlist tidak boleh kosong!'
            return 1
        fi
        download_url="$2"
        target_dir="$HOME/Musics/Playlists"
        
        # Jalankan yt-dlp untuk Playlist
        # Catatan: Hapus '--no-playlist' agar yt-dlp mau mengunduh seluruh isi playlist
        yt-dlp \
            --extract-audio \
            --audio-format mp3 \
            --audio-quality 0 \
        ...
        download_url="$1"
        target_dir="$HOME/Musics"

        # Jalankan yt-dlp untuk Single Video
        yt-dlp \
            --no-playlist \
            --extract-audio \
            --audio-format mp3 \
    ...
    # 3. Proses Auto-Tagging Menggunakan Beets CLI
    if command -v beet &> /dev/null; then
        echo "\n[Beets] Memulai proses auto-tagging pada direktori: $target_dir"
        
        # Penjelasan Flag Beets:
        # -q : Quiet (tidak meminta konfirmasi input manual di terminal)
        # -A : Jangan memindahkan file ke direktori pustaka beets (tetap di folder unduhan)
        # -s : Jika kecocokan rendah, gunakan metadata yang ada (Timid fallback)
        beet import -qAs "$target_dir"
        
        echo "[Beets] Proses auto-tagging selesai!"
    else
        echo "\n[Peringatan] Beets CLI tidak ditemukan. Silakan instal terlebih dahulu menggunakan: pip install beets"
    fi
}
